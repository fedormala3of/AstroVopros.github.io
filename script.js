...
function initBottomNavigation() {
    ...
    if (!navShop || !navReadings || !navCabinet) {
        console.log('Кнопки навигации не найдены по ID, ищем по классу...');
        const navButtons = document.querySelectorAll('.nav-item');
        console.log('Найдено кнопок навигации:', navButtons.length);
        navButtons.forEach((btn, index) => {
            console.log(`Кнопка ${index}:`, btn.textContent, btn.id);
        });
    }
    ...
    // Альтернативный способ - обработка через делегирование событий
    document.addEventListener('click', (e) => {
        if (e.target.closest('#navShop')) {
            e.preventDefault();
            console.log('Клик по кнопке Магазин (делегирование)');
            showPage(shopPage, navShop);
        } else if (e.target.closest('#navReadings')) {
            e.preventDefault();
            console.log('Клик по кнопке Гадания (делегирование)');
            showPage(mainPage, navReadings);
        } else if (e.target.closest('#navCabinet')) {
            e.preventDefault();
            console.log('Клик по кнопке Кабинет (делегирование)');
            showPage(cabinetPage, navCabinet);
        }
    });
    ...
    window.testNavigation = {
        showShop: () => showPage(shopPage, navShop),
        showReadings: () => showPage(mainPage, navReadings),
        showCabinet: () => showPage(cabinetPage, navCabinet)
    };
    
    console.log('Навигация инициализирована. Доступны функции:', Object.keys(window.testNavigation));
}

// Глобальная функция для принудительной инициализации навигации
window.forceInitNavigation = function() {
    console.log('Принудительная инициализация навигации...');
    initBottomNavigation();
    initShopFunctions();
    initCabinetFunctions();
    console.log('Навигация принудительно инициализирована');
};
...
// Резервные функции навигации
function addBackupNavigationFunctions() {
    console.log('Добавляем резервные функции навигации...');
    
    // Простая функция для переключения страниц
    function showPage(pageId, activeButtonId) {
        ...
        if (activeButton) {
            ...
            console.log(`✅ Активирована кнопка: ${activeButtonId}`);
        }
    ...
    if (navShop) {
        navShop.addEventListener('click', (e) => {
            e.preventDefault();
            console.log('Клик по кнопке Магазин (резервный)');
            showPage('shopPage', 'navShop');
        });
        console.log('✅ Резервный обработчик для navShop добавлен');
    }
    
    if (navReadings) {
        navReadings.addEventListener('click', (e) => {
            e.preventDefault();
            console.log('Клик по кнопке Гадания (резервный)');
            showPage('mainPage', 'navReadings');
        });
        console.log('✅ Резервный обработчик для navReadings добавлен');
    }
    
    if (navCabinet) {
        navCabinet.addEventListener('click', (e) => {
            e.preventDefault();
            console.log('Клик по кнопке Кабинет (резервный)');
            showPage('cabinetPage', 'navCabinet');
        });
        console.log('✅ Резервный обработчик для navCabinet добавлен');
    }
    ...
    console.log('✅ Резервные функции навигации добавлены');
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('Проверяем наличие элементов навигации...');
    const navElements = document.querySelectorAll('.nav-item');
    console.log('Найдено элементов навигации:', navElements.length);
    ...
    setTimeout(initEnhancedFeatures, 2000);
    
    // Дополнительная проверка через 3 секунды
    setTimeout(() => {
        console.log('Проверка навигации через 3 секунды...');
        const navShop = document.getElementById('navShop');
        const navReadings = document.getElementById('navReadings');
        const navCabinet = document.getElementById('navCabinet');
        ...
        if (!navShop || !navReadings || !navCabinet) {
            ...
            console.log('Кнопки навигации найдены и готовы к работе');
        }
        
        // Проверяем доступность функций навигации
        console.log('Проверяем доступность функций навигации...');
        console.log(`window.testNavigation: ${window.testNavigation ? '✅ найдена' : '❌ не найдена'}`);
        console.log(`window.forceInitNavigation: ${window.forceInitNavigation ? '✅ найдена' : '❌ не найдена'}`);
        
        // Если функции не найдены, добавляем резервные
        if (!window.testNavigation) {
            console.log('Добавляем резервные функции навигации...');
            addBackupNavigationFunctions();
        }
    }, 3000);
});
