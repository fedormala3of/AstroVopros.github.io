...
async function initApp() {
    ...
    if (tg && tg.colorScheme === 'dark') {
        ...
        if (darkThemeToggle) darkThemeToggle.checked = true;
    }
...
function updateUserStatsDisplay() {
    ...
    if (totalReadings) totalReadings.textContent = userStats.totalReadings;
    if (totalSpent) totalSpent.textContent = userStats.totalSpent;
    if (adsWatched) adsWatched.textContent = userStats.adsWatched;
    if (userLevel) userLevel.textContent = userStats.userLevel;
...
function addEventListeners() {
    ...
    // Кнопки навигации
    if (backToSpreads) {
        backToSpreads.addEventListener('click', showReadingsSection);
        console.log('Back to spreads listener added');
    }
    if (newReading) {
        newReading.addEventListener('click', showReadingsSection);
    ...
    if (backToMainBtn) {
        backToMainBtn.addEventListener('click', showReadingsSection);
    ...
    // Нижнее меню
    if (navShop) {
        navShop.addEventListener('click', showShopSection);
        console.log('Nav shop listener added');
    }
    if (navReadings) {
        navReadings.addEventListener('click', showReadingsSection);
        console.log('Nav readings listener added');
    }
    if (navCabinet) {
        navCabinet.addEventListener('click', showCabinetSection);
        console.log('Nav cabinet listener added');
    }
    
    // Кабинет
    if (uploadAdBtn) {
        uploadAdBtn.addEventListener('click', toggleAdUpload);
        console.log('Upload ad listener added');
    }
    if (viewAdStatsBtn) {
        viewAdStatsBtn.addEventListener('click', showAdStats);
        console.log('View ad stats listener added');
    }
    if (manageAdsBtn) {
        manageAdsBtn.addEventListener('click', manageAds);
        console.log('Manage ads listener added');
    }
    if (submitAdBtn) {
        submitAdBtn.addEventListener('click', submitAd);
        console.log('Submit ad listener added');
    }
    if (darkThemeToggle) {
        darkThemeToggle.addEventListener('change', toggleDarkTheme);
        console.log('Dark theme toggle listener added');
    }
    ...
    if (uploadPlaceholder) {
        ...
        console.log('Upload placeholder listener added');
    }
    if (adFileInput) {
        adFileInput.addEventListener('change', handleFileSelect);
    ...
    document.querySelectorAll('.btn-buy').forEach(button => {
        button.addEventListener('click', (e) => {
            ...
            showError(`Покупка "${itemName}" за ${itemPrice} - функция в разработке`);
        });
    });
...
// Показать секцию гаданий
function showReadingsSection() {
    hideAllSections();
    if (spreadSelection) spreadSelection.style.display = 'block';
    updateNavActive('navReadings');
    updateSpreadAvailability();
}

// Показать секцию магазина
function showShopSection() {
    hideAllSections();
    if (shopSection) shopSection.style.display = 'block';
    updateNavActive('navShop');
}

// Показать секцию кабинета
function showCabinetSection() {
    hideAllSections();
    if (cabinetSection) cabinetSection.style.display = 'block';
    updateNavActive('navCabinet');
    updateUserStatsDisplay();
}
...
function showResult(interpretationText) {
    hideAllSections();
    if (readingResult) readingResult.style.display = 'block';
    if (interpretation) interpretation.innerHTML = formatInterpretation(interpretationText);
}
...
function hideAllSections() {
    ...
    if (cabinetSection) cabinetSection.style.display = 'none';
}
...
// Переключить загрузку рекламы
function toggleAdUpload() {
    if (adUpload) {
        const isVisible = adUpload.style.display !== 'none';
        adUpload.style.display = isVisible ? 'none' : 'block';
    }
}
    ...
    if (file) {
        ...
        if (uploadPlaceholder) {
            uploadPlaceholder.innerHTML = `
                ...
                <p class="upload-hint">Размер: ${fileSize} MB</p>
            `;
        }
    }
}
...
function submitAd() {
    ...
    const reward = adReward ? parseInt(adReward.value) : 50;
    const duration = adDuration ? parseInt(adDuration.value) : 15;
    
    if (!title) {
        showError('Введите название рекламы');
        return;
    }
    
    if (!adFileInput || !adFileInput.files[0]) {
        showError('Выберите файл для загрузки');
        return;
    }
    
    // Симуляция загрузки
    showSuccess('Реклама успешно загружена!');
    
    // Очищаем форму
    if (adTitle) adTitle.value = '';
    if (adDescription) adDescription.value = '';
    if (adReward) adReward.value = '50';
    ...
    if (uploadPlaceholder) {
        uploadPlaceholder.innerHTML = `
            ...
            <p class="upload-hint">Поддерживаются: MP4, AVI, JPG, PNG</p>
        `;
    }
...
// Управление рекламой
function manageAds() {
    showError('Функция управления рекламой в разработке');
}
